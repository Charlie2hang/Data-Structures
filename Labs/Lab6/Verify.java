...
public class Verify {
  ...
  public void randomTrees( int _noNodes, int _nTrees ) {
    ...
    for(int i=0; i<_nTrees; ++i) { 
      ...
      AVLTreeMap avlTreeMap= new AVLTreeMap<>();
      TreeMap bstTreeMap= new TreeMap();
	...
	if (avlTreeMap.get(randKey)==null) { // Needs change !!!
	  ...
	  // insert into BST
      bstTreeMap.put(randKey,null);
	  // next key
	  ++count;
	}
      } while ( count < _noNodes );
      // updateCounters( height( treeAVL ), height( treeBST));
      updateCounters( height(avlTreeMap), height(bstTreeMap)); // Needs change !!!
    }
    return;
  }
   ...
   */
  public <K,V> int height( TreeMap<K,V> bTree ) {
